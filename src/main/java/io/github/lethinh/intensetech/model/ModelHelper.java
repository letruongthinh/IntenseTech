...
public class ModelHelper {
				...
				for (int u = 0; u < uMax; u++) {
					...
					if (!translucent) {
						...
						if (building && !face) // finish current quad
						{
							...
							builder.add(buildSideQuad(format, transform, facing, tint, sprite, uStart, v + off,
									u - uStart));
							building = false;
				...
				for (int v = 0; v < vMax; v++) {
					...
					if (!translucent) {
						...
						if (building && !face) // finish current quad
						{
							...
							builder.add(buildSideQuad(format, transform, facing, tint, sprite, u + off, vStart,
									v - vStart));
							building = false;
						} else if (!building && face) // start new quad
						{
	...
	private static BakedQuad buildQuad(
			VertexFormat format, Optional<TRSRTransformation> transform, EnumFacing side, TextureAtlasSprite sprite,
			int tint,
